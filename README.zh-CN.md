...
- **服务器管理。** 频道、访问令牌、注册用户、封禁（定时或永久，按证书或 IP）、踢人、移动、静音、聋哑、优先发言，以及完整的频道 ACL 编辑器（按组/按用户授权、继承条目、组成员管理）。
...
| 私语 / 喊话 | 向服务器注册语音目标（协议 `VoiceTarget`，id 1–30）并在发送语音时携带该目标：私语给一个或多个用户，或喊话到某频道（可跟随链接频道、包含子频道，并可限制 ACL 组）。入口在目标用户/频道的长按菜单里（自己的长按菜单另有多选用户与选择频道的入口），不设独立按钮；当前目标常显在语音条上方，一键即可恢复普通说话；目标已无接收者时不会发送语音，绝不会退化为频道广播 |
| 音频自测 | 把麦克风声音回环到本机而不是发送出去，用于检查麦、耳机与编解码链路。默认本地回环（不走网络、离线可用），可用开关切换为服务器回环（协议 target 31，由 murmur 原样回送）。仅本会话有效，语音条上方有提示，其他人不会听到 |
...
- 创建、编辑、删除、移动频道——包括临时频道、最大用户数、描述与密码（按地址记住为访问令牌）。
- **ACL 编辑器**：对任何你有写入权限的频道（本频道或根频道有 Write）都可编辑。ACL 标签页逐条编辑作用目标（组、`auth` / `in` / `out` 等内置元组，或按名称查询的注册用户）、各权限位的允许/拒绝矩阵、条目是否应用于本频道与子频道，以及条目的求值顺序（越靠下优先级越高）；继承自父频道的条目以斜体只读列出。组标签页可新建组，并管理其成员、排除成员与继承自父频道的成员。服务端版本不认识的权限行会隐藏（收听与重置用户内容需要 Mumble 1.4.0）。频道密码仍是其上的便捷封装——一条 deny-all 加一条授予 `#password`，仍由频道对话框编辑。
...
│   ├── crypto/             OCB2-AES128（CryptOCB2）、CryptState 与语音加密策略
...
| 关注点 | 方案 |
| --- | --- |
| 控制通道 | 要求 TLS 1.2 及以上（平台支持时使用 TLS 1.3），对应官方客户端的 `TlsV1_2OrLater` 下限；「允许旧版 TLS」开关可为只提供旧协议的服务器加回 TLS 1.0/1.1，但 Android 15 及以上系统会拒绝这两个版本。协商出的版本与加密套件在连接信息中展示 |
...
| 频道访问 | 服务端 ACL，可在应用内完整编辑：按组/按用户对各权限位允许或拒绝、继承自父频道的条目、组成员；频道密码是它之上的便捷封装——一条 deny-all 加一条授予 `#password`，由频道对话框写入 |
...
自动重连、证书固定、允许旧版 TLS、服务器列表自动 ping 及间隔。
...
JVM 测试套件覆盖容易出错的细节：UDP/TCP 编解码、`PacketDataStream` varint 边界、OCB2 组帧、ping 编解码、抖动缓冲行为、VAD 阈值、输出路由规则、频道树维护、ACL 权限判断、ACL 编辑器草稿规则、频道密码处理、封禁、设置迁移。
...
欢迎贡献——构建、测试与发起 Pull Request 的方式见 [CONTRIBUTING.md](CONTRIBUTING.md)。
发起 Pull Request 即表示您同意本项目的[贡献者许可协议](CLA.md)，该协议允许本项目的
维护者与管理者（当前及未来）将您的贡献内容按任意 OSI 批准许可证再许可。
